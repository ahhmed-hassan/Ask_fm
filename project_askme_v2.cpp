...
#include<assert.h>
using namespace std;
// the big picture to not to get lost:
//Our project has the basic User and question class. the user can refer to the questions using vector of their ids
// then the question manager to handle all questions of the system, connecting thread questions with their parent question
// the user manager that manage all users and provide us with the current user who run the system
...
private:
	...
	string name;
	string email;
	int allow_anonymous_questions;	// 0 or 1, maybe it was better to go for boolean here
...
public:
	...
	const map<int, vector<int> >& GetQuestionidQuestionidsTheadToMap() const {
		...
		// Note no setter is provided / returned as const & to be the same in memory and no changes are allowed
	}
...
private:
	// From question id to list of questions IDS on this question (thread questions) -for  All users
	map<int, vector<int>> questionid_questionidsThead_to_map;

	// Map the question id to question object. Let's keep one place ONLY with the object
	//We could've used pointers, but I saw no need
...
public:
	...
	void PrintUserToQuestions(const User &user) const {
		...
		for (const auto &pair : user.GetQuestionidQuestionidsTheadToMap()) {		// pair<int, vector<Question>>
			for (const auto &question_id : pair.second) {		//  vector<Question>
				...
				//using const here isn't optional,as we go for a const function the map::find will return a const iterator
				const Question &question = questionid_questionobject_map.find(question_id)->second;
	...
	// Used in Answering a question for YOU and Deleting it too.
	...
	void AskQuestion(const User &user, const pair<int, int> &to_user_pair) {
		...
		string line;
		getline(cin, line);
		getline(cin, line); // get the last enter
		...
		//Maybe we could've used static variable and decreasign it every time we call the constructor or call
		//The ask question function. or declare the question using another question object (it will be  no need to provide a copy constructor as we have no poitners)
...
public:
	...
	void DoSignUp() {
        ...
        //declaring a data members of the current_user object is a typical job for the user class not for the manager
